...
const currentWordTranslationEl = document.getElementById(
...
function loadSettings() {
  ...
  if (settings.secondLanguageEnabled)
    secondLangEnabledEl.checked = settings.secondLanguageEnabled;
  ...
  if (settings.role) {
    const radio = document.querySelector(
      `input[name="role"][value="${settings.role}"]`,
    );
...
async function renderCategories() {
  ...
  for (const cat of CATEGORIES) {
    ...
    checkbox.checked = savedSettings.categories
      ? savedSettings.categories.includes(cat.id)
      : true;
...
async function renderSimpleWords() {
  ...
  if (!SIMPLE_WORDS[lang1])
    SIMPLE_WORDS[lang1] = await loadJson(`data/simple-words/${lang1}.json`);
  if (showLang2 && !SIMPLE_WORDS[lang2])
    SIMPLE_WORDS[lang2] = await loadJson(`data/simple-words/${lang2}.json`);
  ...
  SIMPLE_WORDS[lang1].forEach((group, groupIndex) => {
    ...
    groupContainer.className =
      "p-px rounded-md border border-rose-300/30 flex flex-wrap gap-1 w-full";

    group.forEach((word, wordIndex) => {
      const chip = document.createElement("div");
      chip.className =
        "px-3 py-1 flex-1 bg-zinc-800 rounded text-md flex flex-col items-center";
...
async function showWord(word) {
  ...
  if (secondLangEnabledEl.checked) {
    ...
    currentWordTranslationEl.textContent =
      TARGET_WORDS[word.category][lang2]?.[idx] || "";
...
function restoreAllSolved() {
  solved.clear();
  solvedStack = [];
  localStorage.setItem("solved", JSON.stringify([]));
  renderCategoryCounts();
}
function restoreLastSolved() {
  if (!solvedStack.length) return;
  const last = solvedStack.pop();
  solved.delete(last);
  localStorage.setItem("solved", JSON.stringify([...solved]));
  renderCategoryCounts();
}
...
document
  ...
  .forEach((r) => r.addEventListener("change", updateRoleVisibility));
...
async function init() {
  ...
  Object.entries(LANG_MAP).forEach(([code, name]) => {
    ...
    languageEl.appendChild(opt1);
    const opt2 = document.createElement("option");
    opt2.value = code;
    opt2.textContent = name;
    secondLangEl.appendChild(opt2);
  });
